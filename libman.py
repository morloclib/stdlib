...
def stdlib_root() -> Path:
    """Return the directory the script lives in -- the folder that holds the
    stdlib module repos (bits/, char/, ..., stdlib/).

    Use `.absolute()`, NOT `.resolve()`: the script is meant to be run from a
    symlink placed in that folder, and `.resolve()` would follow the symlink
    back to the module's own directory (which holds no sibling repos), breaking
    discovery. `.absolute()` keeps the symlink's location.
    """
...
def render_snapshot(pins: List[Tuple[str, str]]) -> str:
    """Render the module-pin snapshot: one `name hash` per line, sorted.

    This is the file mim deposits into an environment's `snapshots/` directory
    and the compiler resolves imports against on demand. It is the same data as
    the umbrella's morloc-deps, in the flat format the compiler parses. The
    filename (set by mim, e.g. `stdlib.txt`) carries NO version -- one
    environment holds one solution, so a snapshot is overwritten in place, never
    versioned alongside another.
    """
    lines = [
        "# stdlib module-pin snapshot -- generated by 'libman.py freeze'; do not edit by hand",
        "# format: <module-name> <git-hash>, one per line",
    ]
    lines += [f"{name} {h}" for name, h in pins]
    return "\n".join(lines) + "\n"
...
def cmd_freeze(args: argparse.Namespace) -> int:
    ...
    # The snapshot lives beside the umbrella's package.yaml, in the stdlib repo,
    # so it is committed and CI can expose it as a release artifact.
    snapshot_path = umbrella.path / SNAPSHOT_NAME
    snapshot_text = render_snapshot(pins)
    ...
    if args.check:
        ...
        current_snapshot = (
            snapshot_path.read_text() if snapshot_path.exists() else ""
        )
        snapshot_ok = current_snapshot == snapshot_text
        if not (missing or extra or mismatched) and snapshot_ok:
            print(f"  {GREEN}stdlib/package.yaml and {SNAPSHOT_NAME} are in sync with submodule HEADs{NC}")
            return 0
        if missing:
        ...
        if not snapshot_ok:
            print(f"  {YELLOW}{SNAPSHOT_NAME} is out of sync{NC} (run: libman.py freeze)")
        return 1

    if args.dry_run:
        print(f"{BOLD}{BLUE}freeze --dry-run{NC} (would write to "
              f"{umbrella.package_yaml} and {snapshot_path})\n")
    ...
    print(f"  {GREEN}wrote{NC}    {snapshot_path.relative_to(stdlib_root().parent.parent)}")
